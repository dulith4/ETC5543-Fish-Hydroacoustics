...
# 1) Compute per-fish quantiles (q60 = median-like)
pf_q60 <- fish_raw |>
  group_by(fishNum, species) |>
  summarise(across(all_of(freq_cols), ~ quantile(.x, 0.6, na.rm = TRUE)), .groups = "drop")

# 2) Effect size across frequencies
...
At approximately **87.5 kHz**, where the per-fish *q60* target strength differs most strongly (Cohen’s d ≈ 0.6), **Lake Trout (LT)** show consistently **higher backscatter** than **Smallmouth Bass (SMB)** (Figure @fig-onefreq-q60).
...
Example of raw **frequency response curves (FRCs)** for one randomly selected **Lake Trout (LT)** and one **Smallmouth Bass (SMB)** in the @fig-frc-quantiles.
...
Compared the recurrent neural network (**RNN**) baseline from the original hydroacoustic study with a set of **H2O AutoML classifiers** trained on progressively feature-enriched inputs.
...
Across models, discrimination performance was consistently high (@tbl-overview).
AUC (TEST) ranged from **0.93–1.00** for the AutoML variants, substantially exceeding the RNN baseline (**AUC = 0.84**).
...
gt(perf_synced) |>
  ...
  tab_options(
    table.font.size           = gt::px(9),   # shrink font
    data_row.padding          = gt::px(1),   # tighter rows
...
Overall, AUC values above 0.93 across all variants indicate consistently strong species discrimination performance.
...
#| tbl-pos: H
library(dplyr); library(purrr); library(stringr); library(readr); library(gt)

# uses results_focus already created in earlier "load-results" chunk
focus_variants <- c("original_blocks","quintiles_allfreq","quintiles_feats","median_allfreq")
...
Hence, report **policy-clipped accuracy** as the main thresholded metric, alongside AUC (TEST).
...
1. **Out-of-Fold (OOF) threshold tuning** — `Analysis/07_oof_threshold_tuning.R`
   ...
   * `median_feats`: **0.750** to **0.667** (drop; clipped to 0.70).
     **Takeaway:** OOF tuning can help when probabilities are well-calibrated (notably `median_allfreq` here), but it can also hurt—supporting use of a **policy window**.
...
The central result stands: **simple, robust fish-level summaries of the FRC combined with leakage-aware training already deliver high, reproducible performance**. The three extensions—OOF thresholding, DL grids, and “plus” features—clarify when extra complexity helps, but they are **not required** to achieve strong accuracy on this dataset. This completes the project.
