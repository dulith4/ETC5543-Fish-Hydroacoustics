...
# choose which metric to decide "best row" by:
# if Acc @ Policy exists, use that; otherwise fall back to Acc @ 0.50
score_for_rank <- if (all(is.na(baselines$`Acc @ Policy`))) {
  baselines$`Acc @ 0.50`
} else {
  dplyr::coalesce(baselines$`Acc @ Policy`, baselines$`Acc @ 0.50`)
}
...
# Pick the row to highlight: prefer Acc @ Policy, else fall back to Acc @ 0.50
...
ts_automl |>
  ...
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE,
                htmltable_class = "fit-table") |>
  row_spec(highlight_idx, bold = TRUE, color = "#0B63CE")   # <- bold + coloured text
...
# Select row to highlight: prefer TEST Acc @ OOF, else TEST Acc @ 0.50
score_for_rank <- dplyr::coalesce(oof$`TEST Acc @ OOF`, oof$`TEST Acc @ 0.50`)
highlight_idx  <- which.max(score_for_rank)
...
oof |>
  ...
  kable_styling(bootstrap_options = c("striped","hover","condensed"),
                full_width = FALSE,
                htmltable_class = "fit-table") |>
  row_spec(highlight_idx, bold = TRUE, color = "#0B63CE")   # highlight best row
...
# choose row to highlight (by TEST Acc @ Policy)
highlight_idx <- which.max(dlgrid$`TEST Acc @ Policy`)
